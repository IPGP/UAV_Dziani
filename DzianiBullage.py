...
def analyser_video_srt(path_video):
    ...
    if extension == ".mov":
        ...
        try:
            cmd = ["ffmpeg", "-y", "-i", str(path_video), "-map", "0:s:0", path_srt]
            subprocess.run(
                cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
        except subprocess.CalledProcessError:
            raise RuntimeError(
                f"Impossible d'extraire la piste de sous-titres de {path_video.name}"
            )
        ...
        if not path_srt.exists():
            raise FileNotFoundError(
                f"Fichier SRT externe introuvable pour {path_video.name} : {path_srt}"
            )
...
class DzianiBullage:
    ...
    def __post_init__(self):
        ...
        self.sensor_height_mm, self.sensor_width_mm, self.focal_distance_mm = eval(
            donnees["SENSOR_DATA"]
        )
        ...
        self.DETECTION_PARAMETERS = dict(
            maxCorners=self.NB_BUBBLES, qualityLevel=0.1, minDistance=0.5, blockSize=10
        )
        self.TRACKING_PARAMETERS = dict(
            winSize=(15, 15),
            maxLevel=2,
        ...
        self.output_path = (
            self.output_path
            / f"{self.line_number}_resultats_{self.window_size_seconds}s_{self.windows_shift_seconds}s_{self.date_video}_{self.input_video_filename}"
        )
        ...
        self.results_np_speeds_filepath = (
            self.output_path / f"results_speeds_{self.tag_file}.npy"
        )

        self.results_grid_X_filepath = (
            self.output_path / f"results_grid_X_{self.tag_file}.npy"
        )
        self.results_grid_Y_filepath = (
            self.output_path / f"results_grid_Y_{self.tag_file}.npy"
        )
        self.results_smoothed_grid_speeds_filepath = (
            self.output_path / f"results_smoothed_grid_speeds_{self.tag_file}.npy"
        )
        ...
        alti_rtk_home, hauteur_moyenne_drone, self.alti_abs_drone = analyser_video_srt(
            self.video_path
        )
        self.distance_lac = self.alti_abs_drone - self.alti_abs_lac
    ...
    def get_gsd(self):
        ...
        self.GSD_Calcul_H = (
            ...
            * self.sensor_height_mm
            / (self.focal_distance_mm * self.frame_height)
        )
        self.GSD_Calcul_W = (
            self.distance_lac
            * self.sensor_width_mm
            / (self.focal_distance_mm * self.frame_width)
        )
        print(
            f"{self.sensor_height_mm=}\t{self.sensor_width_mm=}\t{self.frame_height=}\t{self.frame_width=}\t{self.GSD_Calcul_H=}\t{self.GSD_Calcul_W=}\t {min(self.GSD_Calcul_H,self.GSD_Calcul_W)=}"
        )
        self.GSD_Calcul = min(self.GSD_Calcul_H, self.GSD_Calcul_W)

        print("####################################################")
        print(
            f"le GSD calculé du film {self.input_video_filename} et de numero {self.line_number}  est : {min(self.GSD_Calcul_H,self.GSD_Calcul_W)}"
        )
        print("####################################################")
        detection_area_pixels = np.pi * ((self.detection_radius) ** 2)
        detection_area_meters = detection_area_pixels * (self.GSD_Calcul**2)
        print(
            f"L'aire de la zone de detection est de {detection_area_pixels:.2f} pixels"
        )
    ...
    def speed_to_color(self, speed):
        ...
        norm = plt.Normalize(
            self.VITESSE_MIN_CLASSES_VITESSES,
            self.VITESSE_MAX_CLASSES_VITESSES,
            clip=True,
        )
    ...
    def draw_color_scale(
        self,
        frame,
        position_echelle_couleur,
        hauteur_echelle_couleur,
        largeur_echelle_couleur,
    ):
        """
        ...
        position_echelle_couleur = (
            position_echelle_couleur[0],
            position_echelle_couleur[1] + 100,
        )  # Descend l'échelle de 100 pixels

        for i in range(hauteur_echelle_couleur):
            # Inverser l'échelle de vitesse
            speed = self.VITESSE_MAX_CLASSES_VITESSES - (
                self.VITESSE_MAX_CLASSES_VITESSES - self.VITESSE_MIN_CLASSES_VITESSES
            ) * (i / hauteur_echelle_couleur)
            color = self.speed_to_color(speed)
            cv2.line(
                frame,
                (position_echelle_couleur[0], position_echelle_couleur[1] + i),
                (
                    position_echelle_couleur[0] + largeur_echelle_couleur,
                    position_echelle_couleur[1] + i,
                ),
                color,
                1,
            )
        ...
        cv2.putText(
            frame,
            f"{self.VITESSE_MIN_CLASSES_VITESSES} m/s",
            (
                position_echelle_couleur[0] + largeur_echelle_couleur + 10,
                position_echelle_couleur[1] + hauteur_echelle_couleur,
            ),
        ...
        cv2.putText(
            frame,
            f"{self.VITESSE_MAX_CLASSES_VITESSES} m/s",
            (
                position_echelle_couleur[0] + largeur_echelle_couleur + 5,
                position_echelle_couleur[1],
            ),
            cv2.FONT_HERSHEY_SIMPLEX,
            1,
            (255, 255, 255),
    ...
    def draw_legend(self, img):
        ...
        colors = {
            f"Vitesses faibles (< {self.BORNE_INF_GRAPH:.2f} m/s)": (
                0,
                255,
                255,
            ),  # Jaune
            f"Vitesses moyennes(>={self.BORNE_INF_GRAPH:.2f} - <{self.BORNE_SUP_GRAPH:.2f} m/s)": (
                0,
                165,
                255,
            ),  # Orange
            f"Vitesses elevees(>= {self.BORNE_SUP_GRAPH:.2f} m/s)": (
                0,
                0,
                255,
            ),  # Rouge
        ...
        for label, color in colors.items():
            ...
            cv2.rectangle(
                img, (50, position_y_debut), (200, position_y_debut + 100), color, -1
            )
            # Ajouter le texte descriptif
            cv2.putText(
                img,
                label,
                (210, position_y_debut + 65),
                cv2.FONT_HERSHEY_SIMPLEX,
                1.5,
                (0, 0, 0),
                3,
    ...
    def tracer_vitesse_vs_temps(
        self, numero_bulles_triees, speed_matrix, time_steps, debut_echantillonnage
    ):

        fig, ax = plt.subplots()
        normalisation = plt.Normalize(
            vmin=self.VITESSE_MIN_CLASSES_VITESSES,
            vmax=self.VITESSE_MAX_CLASSES_VITESSES,
        )  # Normalisation des données de vitesse pour l'échelle de couleur

        color_mesh = ax.pcolormesh(
            time_steps,
            np.arange(len(numero_bulles_triees)),
            speed_matrix,
            cmap=self.colormap,
            norm=normalisation,
            shading="auto",
        )
        color_bar_echelle = plt.colorbar(
            color_mesh, ax=ax
        )  # Ajout d'une barre de couleurs à l'échelle
    ...
    def save_trajet(
        ...
        masque_suivi,
        frame,
        points_encore_suivis,
        frame_count,
        debut_echantillonnage,
    ):
        ...
        cv2.putText(
            frame, time_text, (70, 70), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 7
        )
        cv2.putText(
            frame,
            f"Nombre de points suivis: {len(points_encore_suivis)}",
            (70, 150),
            cv2.FONT_HERSHEY_SIMPLEX,
            2,
            (0, 0, 0),
        ...
        cv2.putText(
            frame,
            f"{self.date_video} - {self.input_video_filename}",
            (70, 230),
            cv2.FONT_HERSHEY_SIMPLEX,
            2,
            (0, 0, 0),
        ...
        if self.SAVE_PLOTS:
            filename = (
                f"Trajets_des_bulles_{self.tag_file}_{debut_echantillonnage:03}.png"
            )
            filepath = self.output_path / filename
            cv2.imwrite(filepath, img)
            self.save_lowres_image(
                f"Trajets_des_bulles_{self.tag_file}_{debut_echantillonnage:03}", img
            )
    ...
    def calculer_vitesse_bulles(self, debut_echantillonnage):
        ...
        """
        print(
            f"{self.input_video_filename} Calcul calculer_vitesse_bulles for offset {debut_echantillonnage:03} avec une fenetre de {self.window_size_seconds} secondes start"
        )
        ...
        video_file.set(
            cv2.CAP_PROP_POS_FRAMES, debut_echantillonnage * self.frames_per_second
        )
        ...
        if not frame_available:
            print(
                f"Erreur de lecture de la première frame de {self.input_video_filename}"
            )
            video_file.release()
            sys.exit()
        ...
        masque_detection = np.zeros(
            (self.frame_height, self.frame_width), dtype=np.uint8
        )  # Crée un masque de la même taille que l'image, mais en niveaux de gris
        # Dessine un cercle plein = cercle de détection sur le masque avec une valeur de 255 (blanc)
        cv2.circle(
            masque_detection,
            self.detection_center,
        ...
        if debut_echantillonnage == TEMPS_GRAPHIQUES_PREMIERE_FENETRE:
            ...
            filename = (
                f"Cercle_detection_{self.tag_file}_{debut_echantillonnage:03}.png"
            )
            filepath = self.output_path / filename
            cv2.imwrite(filepath, first_frame_copy)
            self.save_lowres_image(
                f"Cercle_detection_{self.tag_file}_{debut_echantillonnage:03}",
                first_frame_copy,
            )

        # Détermination des caractéristiques de détection
        first_frame_gray = self.frame_to_BGR2GRAY(first_frame)
        positions_initiales = cv2.goodFeaturesToTrack(
            first_frame_gray, mask=masque_detection, **self.DETECTION_PARAMETERS
        )
        ...
        speed_m_per_sec_par_trajet = (
            {}
        )  # Dictionnaire où chaque trajet correspond à une liste qui contient les vitesses prises par chaque point du trajet en m/s
        indices_particules_immobiles = (
            []
        )  # Indices des points qui n'ont pas beaucoup bougé pendant la fenetre
        ...
        all_speeds_m_per_sec = (
            []
        )  # Liste pour stocker les vitesses en m/s pour chaque point
        ...
        nb_shift_total = int(
            ...
            - self.window_size_seconds / self.windows_shift_seconds
        )
        ...
        t = trange(
            frames_per_window,
            desc=f"{debut_echantillonnage:03} ",
        ...
        for frame_count in t:
            ...
            positions_suivies, statuts, _ = cv2.calcOpticalFlowPyrLK(
                previous_frame_gray,
                frame_gray,
                positions_initiales,
                None,
            ...
            if (
                len(masque_suivi.shape) == 2 or masque_suivi.shape[2] == 1
            ):  # mask est en niveaux de gris
                masque_suivi = cv2.cvtColor(masque_detection, cv2.COLOR_GRAY2BGR)

            for i, (new, old) in enumerate(
                zip(points_encore_suivis, points_initiaux_encore_suivis)
            ):
        ...
        for indice, particule in points_suivis_bruts.items():
            ...
            for trajet in particule:
                if trajet.speed >= self.MAX_SPEED_ms:
                    ...
                    break
        print(
            f"Avec max_speed = {self.MAX_SPEED_ms}m/s <=> {self.MAX_SPEED_ms*self.frame_time/self.GSD_Calcul}px/frame on retire {nb_exces_de_vitesse}/{len(points_suivis_bruts)} particules"
        )
        ...
        print(
            f"Pour {self.MIN_DIST_m=}, {nb_particules_immobiles}/{len(distances_totales)} particules \\\
        ...
        for indice, particule in points_suivis_bruts.items():
            for trajet in particule:
                if (indice not in indices_particules_immobiles) and (
                    indice not in indices_particules_high_speed
                ):
                    ...
                    if (
                        trajet.debut_echantillonnage
                        == TEMPS_GRAPHIQUES_PREMIERE_FENETRE
                    ):
                        # print(f'trajet {indice} {int(x)},{int(y)},{speed}')
                        color = self.speed_to_color(trajet.speed)
                        cv2.circle(
                            masque_suivi,
                            (int(trajet.x), int(trajet.y)),
                            rayon_cercle_largeur_ligne,
                            color,
                            -1,
                        )
        ...
        if debut_echantillonnage == TEMPS_GRAPHIQUES_PREMIERE_FENETRE and (
            self.SAVE_PLOTS or self.DISPLAY_PLOTS
        ):
            self.save_trajet(
                masque_suivi,
                frame,
        ...
        print(
            f"{debut_echantillonnage} {len(all_X)=}\t{len(all_Y)=}\t{len(all_speeds_m_per_sec)=}\t{len(speed_m_per_sec_par_trajet)=}"
        )

        if (
            len(all_X) == 0
            and len(all_Y) == 0
            and len(all_speeds_m_per_sec) == 0
            and len(speed_m_per_sec_par_trajet) == 0
        ):
        ...
        for i, speed_list_per_frame in speed_m_per_sec_par_trajet.items():
            if len(speed_list_per_frame) >= self.frames_per_second:
                ...
                ma_speeds = self.calculate_moving_average(
                    speed_list_per_frame, self.frames_per_second
                )
                # ma_speeds[::X] prend un point tout les X points
                vitesses_moyennes[i] = ma_speeds[
                    :: self.frames_per_second
                ]  # Extraire une moyenne tous les fps frames
        ...
        longest_length = max(
            (len(speeds) for speeds in vitesses_moyennes.values()), default=-9999
        )

        if longest_length == -9999:
            print("-------------------------------------------------------------------")
            print(
                f"{debut_echantillonnage:03} le nb de vitesse calculees dans vitesses_moyennes[0] est {len(vitesses_moyennes[0])}"
            )
            print(
                f"{debut_echantillonnage:03} mais il en faudrait {self.window_size_seconds}"
            )
        ...
        vitesse_moyenne_totale = {
            bubble_id: np.mean(speeds)
            for bubble_id, speeds in speed_m_per_sec_par_trajet.items()
        }
        ...
        sorted_bubble_ids = sorted(
            vitesse_moyenne_totale, key=vitesse_moyenne_totale.get
        )

        longest_length = max(
            len(speeds) for speeds in speed_m_per_sec_par_trajet.values()
        )
        ...
        if debut_echantillonnage == TEMPS_GRAPHIQUES_PREMIERE_FENETRE:
            self.tracer_vitesse_vs_temps(
                sorted_bubble_ids, speed_matrix, time_steps, debut_echantillonnage
            )
    ...
    def video_file_analysis(self):
        ...
        array_arguments_for_calculer_vitesse_bulles = list(
            range(
                TEMPS_GRAPHIQUES_PREMIERE_FENETRE,
                self.movie_length_seconds - self.window_size_seconds,
                self.windows_shift_seconds,
            )
        )
        ...
        with Pool(processes=self.cpu_nb) as pool:
            ...
            results_local = pool.map(
                self.calculer_vitesse_bulles,
                array_arguments_for_calculer_vitesse_bulles,
            )
        ...
        with Timer(
            text="{name}: {:.4f} seconds", name="=> Conversion en tableaux NumPy"
        ):
    ...
    def process_cell(self, args):
        ...
        if cell_density > 0:
            ...
            sample_size = min(
                max_sample_size,
                max(1, int(target_density / (cell_density / np.mean(density) + 1))),
            )

            if len(cell_positions_X) > sample_size:
                indices = np.random.choice(
                    len(cell_positions_X), size=sample_size, replace=False
                )
    ...
    def interpolation(self):
        ...
        with Timer(
            ...
            name="=> Echantillonnage des données pour l interpolation en fonction de la densité des points",
        ):
        ...
        with Timer(
            text="{name}: {:.4f} seconds",
            name="=> Calculer la densité des points dans chaque cellule",
        ):
            density, _, _ = np.histogram2d(
                positions_X, positions_Y, bins=[x_edges, y_edges]
            )

        # Définir un nombre cible et un nombre max de points à échantillonner
        with Timer(
            text="{name}: {:.4f} seconds",
            name="=> Définir un nombre cible et un nombre max de points à échantillonner",
        ):
            total_points = len(positions_X)
            target_density = total_points / (num_cells**2)  # nombre cible
            max_sample_size = int(
                np.ceil(target_density / 1000) * 1000
            )  # nombre max = nombre cible arrondi au millier supérieur
        ...
        with Timer(text="{name}: {:.4f} seconds", name="=> Parcourir chaque cellule"):
            args = [
                ...
                for i, j in itertools.product(
                    range(len(x_edges) - 1), range(len(y_edges) - 1)
                )
            ]
            ...
            sampled_positions_X_bis, sampled_positions_Y_bis, sampled_speeds_bis = zip(
                *results
            )
            sampled_positions_X = np.array(
                [item for sublist in sampled_positions_X_bis for item in sublist]
            )
            sampled_positions_Y = np.array(
                [item for sublist in sampled_positions_Y_bis for item in sublist]
            )
            sampled_speeds = np.array(
                [item for sublist in sampled_speeds_bis for item in sublist]
            )

        # Créer une carte des points échantillonnés à interpoler en gradient de couleur
        with Timer(
            text="{name}: {:.4f} seconds",
            name="=> Créer une carte des points échantillonnés à interpoler en gradient de couleur",
        ):
        ...
        with Timer(
            text="{name}: {:.4f} seconds",
            name="=> Créer le meshgrid pour l interpolation",
        ):
            resolution_x = 200
            resolution_y = 200
        ...
        with Timer(
            text="{name}: {:.4f} seconds", name="=> Interpolation sur le meshgrid "
        ):
        ...
        with Timer(
            text="{name}: {:.4f} seconds",
            name="=> Appliquer un filtre de moyenne pour lisser le signal",
        ):
            sigma = 1.5  # Paramètre de lissage
            self.smoothed_grid_speeds = ndimage.gaussian_filter(
                grid_speeds, sigma=sigma
            )
    ...
    def calcul_centre(
        ...
        cv2.circle(
            masque_detection, detection_center, detection_diameter, 255, thickness=-1
        )
        ...
        positions_initiales = cv2.goodFeaturesToTrack(
            previous_frame_gray, mask=masque_detection, **self.DETECTION_PARAMETERS
        )

        # nb_shift_total =int(movie_length_seconds - window_size_seconds/windows_shift_seconds)
        table_colors = (
            plt.colormaps.get_cmap("plasma").resampled(nb_total_decalage).colors
        )

        # Boucle de traitement pour chaque frame jusqu'à atteindre frames_per_window
        t = trange(
            FRAMES_TO_COMPUTE,
            desc=f"{decalage:03} frame analysis ",
            position=numero_decalage,
            leave=True,  # the progress bar will be cleared up and the cursor position unchanged when finished
        ...
        for frame_count in t:
            ...
            positions_suivies, statuts, err = cv2.calcOpticalFlowPyrLK(
                previous_frame_gray,
                frame_gray,
                positions_initiales,
                None,
            ...
            for i, (new, old) in enumerate(
                ...
                distance = np.sqrt(
                    (x_new_point - x_old_point) ** 2 + (y_new_point - y_old_point) ** 2
                )
        ...
        if False:
            ...
            n, bins, patches = plt.hist(
                array_distance, color="lightgreen", ec="black", bins=100
            )
        ...
        for key, value in distances_totales.items():
            if value < seuils_classes_distances[0]:
                ...
                if AFFICHAGE_TRAJETS:
                    for point in trajets[key]:
                        ...
                        cv2.circle(
                            masque_suivi,
                            (int(x_new_point), int(y_new_point)),
                            rayon_cercle_largeur_ligne,
                            color,
                            -1,
                        )
            ...
            if (
                distances_totales[key] > seuils_classes_distances[0]
                and distances_totales[key] < seuils_classes_distances[1]
            ):
                # Fushia
                color = (255, 0, 255)
                if AFFICHAGE_TRAJETS:
        ...
        for couple in [
            (trajets_coefs_all[i], trajets_coefs_all[i + 1])
            for i in range(0, len(trajets_coefs_all), 2)
        ]:
            a = couple[0]
            b = couple[1]
        ...
        for solution in solutions:
            ...
            if (
                x > 0
                and x < self.frame_width
                and y > 0
                and y < self.frame_height
                and dist(solution, detection_center) < 1500
            ):
    ...
    def calcul_centre_panache(self):
        ...
        decalage_list = list(
            range(0, movie_length_seconds - window_size_seconds, windows_shift_seconds)
        )
        array_arguments_for_calcul_centre = [
            (
                seuils_classes_distances,
                window_size_seconds,
                decalage,
            ...
            for index, decalage in enumerate(decalage_list)
        ]
        ...
        with Pool(
            processes=cpu_nb, initargs=(RLock(),), initializer=tqdm.set_lock
        ) as pool:
            # Utiliser pool.map pour appliquer la fonction calculer_vitesse_bulles à chaque élément
            #  de la array_arguments_for_calculer_vitesse_bulles
            barycenters = pool.starmap(
                self.calcul_centre, array_arguments_for_calcul_centre
            )
        ...
        print(
            f"ultimate_center for {self.date_video} {self.video_path} {ultimate_center[0]},{ultimate_center[1]}"
        )
        ...
        print(
            f"{self.tag_file}\tcentre zone de detetion {int(final_center[0])}, {int(final_center[1])}"
        )
    ...
    def calcul_largeur_panache(self):
        ...
        # Placer le centre
        ax.scatter(
            self.center_interpolation[1],
            self.center_interpolation[0],
            color="red",
            label="Center",
            zorder=5,
        )
        ...
        fig.savefig(
            self.output_path / f"find_center_{self.tag_file}.png",
            dpi=dpi,
            bbox_inches="tight",
            pad_inches=0,
        )
        ...
        for ray_X, ray_Y in zip(coords_X, coords_Y):
            ax.plot(
                ray_Y, ray_X, color="gray", alpha=0.9, linewidth=0.7
            )  # Attention aux coordonnées : Y en premier pour imshow
        ...
        for angle in angles:
            ...
            for r in range(1, self.rayon_max_panache + 1):
                ...
                if 0 <= x_index < speeds.shape[0] and 0 <= y_index < speeds.shape[1]:
                    ...
                    distance_m = np.sqrt(
                        (x_m - center_x_m) ** 2 + (y_m - center_y_m) ** 2
                    )
        ...
        for ray_distances, ray_speeds in zip(distances_along_rays, speeds_along_rays):
            ...
            interp_func = interp1d(
                ray_distances, ray_speeds, bounds_error=False, fill_value=np.nan
            )
        ...
        smoothing_spline = UnivariateSpline(
            distance_grid, median_speeds, s=0.001
        )  # Le paramètre "s" contrôle la douceur
        ...
        for ray_distances, ray_speeds in zip(distances_along_rays, speeds_along_rays):
            ax.plot(
                ray_distances, ray_speeds, alpha=0.2, color="grey", linewidth=0.7
            )  # Courbes individuelles avec transparence
        ...
        ax.plot(
            ...
            color="black",
            linewidth=2,
            label="Courbe médiane",
        )

        # Tracer la courbe médiane lissée
        ax.plot(
            distance_grid,
            smoothing_spline(distance_grid),
            color="red",
            linewidth=2,
            label="Courbe médiane lissée",
        )
        ...
        ax.axvline(
            ...
            color="black",
            linestyle="--",
            label=f"Max à {max_distance:.2f} m",
        )
        ax.scatter([max_distance], [max_speed], color="white", zorder=5)
        t = ax.text(
            max_distance,
            max_speed + 0.05,
            f"Rayon = {max_distance:.2f} m  - {max_speed:.2f}m/s",
            color="black",
            fontsize=20,
        ...
        ax.set_title(
            f"Vitesse en fonction de la distance au centre\n{self.line_number} - {self.date_video} - {self.input_video_filename}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DzianiBullage necessite le numéro de ligne a traiter comme argument."
    )
    ...
    parser.add_argument(
        "numero_ligne",
        type=int,
        help="Le numéro de ligne à traiter qui doit être un entier.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="flag pour trouver le centre du panache",
    )
    parser.add_argument(
        "--file_analysis",
        "-a",
        default=False,
        action="store_true",
        help="flag pour faire l'analyse du fichier",
    )
    parser.add_argument(
        "--interpolation",
        "-i",
        default=False,
        action="store_true",
        help="flag pour faire l'interpolation",
    )
    parser.add_argument(
        "--largeur_panache",
        "-l",
        default=False,
        action="store_true",
        help="flag pour calculer la largeur du panache",
    )
    ...
    dziani_bullage = DzianiBullage(
        google_sheet_id=google_sheet_id,
        line_number=numero_ligne_a_traiter,
        output_path=output_path,
        root_data_path=root_data_path,
        window_size_seconds=duree_fenetre_analyse_seconde,
